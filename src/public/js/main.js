...
// Handle View of Arrival Product By Category in id arrivals
document.addEventListener("DOMContentLoaded", function () {
    const navLinks = document.querySelectorAll(".ec-pro-tab-nav .nav-link");
    const productRows = document.querySelectorAll(".category-products");
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    quickViewButtons.forEach(button => {
        button.addEventListener('click', function () {
            // Lấy dữ liệu từ nút
            const productName = button.getAttribute('data-product-name') || 'No Name';
            const productDescription = button.getAttribute('data-product-description') || 'No Description';
            const productPrice = button.getAttribute('data-product-price') || '0.00';
            const productOldPrice = button.getAttribute('data-product-old-price') || productPrice;
            const productImages = JSON.parse(button.getAttribute('data-product-images') || '[]');

            // Cập nhật modal
            const modal = document.querySelector('#ec_quickview_modal');
            modal.querySelector('.ec-quick-title').textContent = productName;
            modal.querySelector('.ec-quickview-desc').textContent = productDescription;
            modal.querySelector('.new-price').textContent = `$${productPrice}`;
            modal.querySelector('.old-price').textContent = `$${productOldPrice}`;

            // Cập nhật hình ảnh chính và thumbnail
            const mainImageContainer = modal.querySelector('.qty-product-cover');
            const thumbnailsContainer = modal.querySelector('.qty-nav-thumb');

            // Xóa nội dung cũ
            mainImageContainer.innerHTML = '';
            thumbnailsContainer.innerHTML = '';

            // Thêm hình ảnh chính đầu tiên
            const mainImageDiv = document.createElement('div');
            mainImageDiv.className = 'qty-slide';
            mainImageDiv.innerHTML = `
                <img class="img-responsive main-image" src="${productImages[0]}" alt="${productName}">
            `;
            mainImageContainer.appendChild(mainImageDiv);

            // Thêm các hình ảnh thu nhỏ (thumbnails)
            productImages.forEach((image, index) => {
                const thumbDiv = document.createElement('div');
                thumbDiv.className = 'qty-slide';
                thumbDiv.innerHTML = `
                    <img class="img-responsive" src="${image}" alt="${productName} Thumbnail ${index + 1}">
                `;
                thumbnailsContainer.appendChild(thumbDiv);

                // Thêm sự kiện click để thay đổi hình ảnh chính khi nhấn vào thumbnail
